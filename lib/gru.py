...
from lib.latent_ode_helper import GRU


class GRUWrapper(nn.Module):
    def __init__(self, latent_dim, gru, out):
        super().__init__()
    ...
    def reconstruct(self, x, features, mask):
        ...
        for i in range(x.size(1)):
            ...
            h_rnn = self.gru(enc_data[:, i, :], h)

            if mask is not None:
                curr_mask = mask[:, i].view(-1, 1)
                h = h_rnn * curr_mask + h * (1 - curr_mask)
...
def load_model_gru(all_features, config_dict):
    ...
    return GRUWrapper(latent_dim, gru, out)
